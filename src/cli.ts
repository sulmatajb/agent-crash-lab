...
async function main() {
  const { values, positionals } = parseArgs({ allowPositionals: true, options: {
    ...
    'hermes-python': { type: 'string' }, 'hermes-profile': { type: 'string' }, model: { type: 'string' }, provider: { type: 'string' },
  ...
  if (command === 'evaluate' || command === 'probe') {
