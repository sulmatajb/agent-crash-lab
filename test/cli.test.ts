...
test('CLI outputs machine-readable multi-seed results and meaningful exit codes',()=>{
  const good=spawnSync(process.execPath,[cli,'test','--agent','careful','--runs','3','--json'],{encoding:'utf8'});assert.equal(good.status,0,good.stderr);assert.equal(JSON.parse(good.stdout).summary.passed,21);
