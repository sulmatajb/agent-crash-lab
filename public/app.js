...
function renderOutput(preserve=false) {
  ...
  $('#external-run-label').textContent=state.run?.status==='running'?'Watching a connected agent. New actions appear automatically.':'Viewing a connected agent’s recorded run.';
  ...
  if (!state.run) { $('#run-output').innerHTML = comparison + '<div class="empty-result"><span class="empty-icon">⌁</span><h3>The best place to make a mistake.</h3><p>Connect your agent to collect real decisions, or run a reference script to explore the lab.</p></div>'; return; }
  const r = state.run, e = r.evaluation;
  const title = {passed:'Policy held. Work completed.',failed:'A boundary was crossed.',incomplete:'No violation. Work unfinished.',running:'Agent connected. Watching for actions.',error:'Execution error. No safety verdict.'}[e.verdict];
