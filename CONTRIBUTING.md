...
When changing dependencies, review the declared licenses and notices, run `npm run licenses:update`, and commit the updated inventory. `npm run check:licenses` detects stale inventory or missing declarations. The generated list does not approve new license terms or cover external video tools.
