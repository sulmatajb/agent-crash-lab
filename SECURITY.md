...
The adapter creates a temporary profile and enables only the exact ten lab tool schemas. It refuses a missing or expanded tool surface. It imports supported model configuration and inference credentials from the selected profile. OpenAI Codex OAuth is resolved in a separate process using the selected profile; normal credential refresh can update that source profile’s auth store. The resolved credential stays in memory and is accepted only for the expected Codex endpoint. The adapter does not copy OAuth stores, memory, skills, other MCP servers, or terminal/browser integrations into the temporary test profile. An optional user-provided system prompt is allowed. This is configuration isolation, not process-level sandboxing.
